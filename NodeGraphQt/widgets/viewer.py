...
class NodeViewer(QtWidgets.QGraphicsView):
    ...
    def __init__(self, parent=None, undo_stack=None):
        ...
        self._cursor_text.setFlag(QtWidgets.QGraphicsTextItem.GraphicsItemFlag.ItemIsSelectable, False)
    ...
    def _build_context_menus(self):
        ...
        """
        # "node context menu" disabled by default and enabled when an action
        ...
        # set up the undo and redo actions.
    ...
    def mouseMoveEvent(self, event):
        ...
        if self.LMB_state and self._rubber_band.isActive:
            ...
            if max(rect.width(), rect.height()) > 5:
                ...
                self._rubber_band.setGeometry(rect)
                self.scene().setSelectionArea(path, QtCore.Qt.ItemSelectionOperation.AddToSelection)
                self.scene().update(map_rect)
    ...
    def wheelEvent(self, event):
        ...
        pos = QtCore.QPoint(event.position().x(), event.position().y())
    ...
    def _validate_reject_connection(self, from_port, to_port):
        """
        Check if a pipe connection is NOT allowed if there are a constraints set
        on the ports.
